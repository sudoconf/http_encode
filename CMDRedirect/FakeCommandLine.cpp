...
LPWSTR WINAPI InlineGetCommandLineW(VOID)
{
	...
	if (NULL == pProcessParameters) {
		Global::Log.PrintW(LOGOutputs, L"[% 5u] Init process parameters filed.", GetCurrentProcessId());
...
bool Hook::StartCommandLineHook()
{
	...
	for (int i = 0; i < count(ptszHitProcessNameLists); i++) {
		if (NULL != wcsistr(pProcessParameters->ImagePathName.Buffer, ptszHitProcessNameLists[i]) && NULL == Global::pfnGetCommandLineW) {
